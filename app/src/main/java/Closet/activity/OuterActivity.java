...
import Closet.ViewPagerAdapter.ViewPagerAdapter_Outer;

public class OuterActivity extends AppCompatActivity implements View.OnClickListener{
    ViewPager2 viewPager2; //뷰페이저
    ViewPagerAdapter_Outer viewPagerAdapter; //뷰페이저 어뎁터
    TabLayout tabLayout; //텝 레이아웃
    ArrayList<Data_Type> mdata; //데이터 모델
    Context context;
    public static OuterActivity activity;
    @Override
...
//        tabLayout.addTab(tabLayout.newTab().setText("C"));
        ...
        viewPagerAdapter = new ViewPagerAdapter_Outer(context ,mdata); //뷰페이저 어뎁터 생성
        ...
        TabLayoutMediator tabLayoutMediator = new TabLayoutMediator(tabLayout, viewPager2, true, new TabLayoutMediator.TabConfigurationStrategy() {
            ...
            public void onConfigureTab(@NonNull TabLayout.Tab tab, int position) {// 텝 레이아웃이랑 연결
                switch (position){
                    case 0:{
                        tab.setText("가디건"); //텝레이아웃 상단 타이틀 제목 설정
                        break;
                    }
                    case 1:{
                        tab.setText("후리스");
